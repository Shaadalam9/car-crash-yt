...
class Maps:
    ...
    def mapbox_map_footage(self, df: pd.DataFrame, *,
        ...
        # Plotly map traces require numeric coordinate columns.  Polars to Pandas
        # conversion may preserve latitude and longitude as string dtypes, so coerce
        # them explicitly and discard missing or invalid geographic coordinates.
        ...
        df_plot = df_plot[
            df_plot["lat"].between(-90, 90)
            & df_plot["lon"].between(-180, 180)
        ].copy()

        if df_plot.empty:
            logger.warning("No valid latitude/longitude values available for mapbox_map_footage.")
            return
    ...
    def mapbox_map(self, df, density_col=None, density_radius=30, hover_data=None, hover_name=None,
        ...
        if missing_columns:
            raise KeyError(f"Missing required map columns: {sorted(missing_columns)}")

        df_plot = df.copy()

        # Plotly 7 uses MapLibre based map traces.  Coordinates must be numeric
        # because Plotly computes their mean when it determines the initial view.
        df_plot["lat"] = pd.to_numeric(df_plot["lat"], errors="coerce")
        df_plot["lon"] = pd.to_numeric(df_plot["lon"], errors="coerce")
        df_plot = df_plot.dropna(subset=["lat", "lon"]).copy()
        df_plot = df_plot[
        ...
        if df_plot.empty:
            logger.warning("No valid latitude/longitude values available for mapbox_map.")
            return

        # Draw scatter map if no density column is provided.
        if not density_col:
            if "continent" not in df_plot.columns:
                raise KeyError("Missing column: continent")
        ...
        # Draw density map if density column is provided.
            ...
            if density_col not in df_plot.columns:
                raise KeyError(f"Missing column: {density_col}")

            df_plot[density_col] = pd.to_numeric(df_plot[density_col], errors="coerce")
            df_density = df_plot.dropna(subset=[density_col]).copy()

            if df_density.empty:
                logger.warning(f"No valid numeric values available for density column: {density_col}")
                return
            ...
            fig = px.density_map(
                ...
                zoom=2.5,  # Initial zoom level for density view.
                center=dict(
                    lat=float(df_density["lat"].mean()),
                    lon=float(df_density["lon"].mean()),
                ),
                map_style="carto-positron",
                hover_data=hover_data,
                hover_name=hover_name,
